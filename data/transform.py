...
def revert_objects(df): 
    ...
    for row in df.iterrows():
        print("ROW", row[0], "of", len(df), "rows")
        product_id = row[1]['product_id']
        
        # verify if product_id is already in the new_df 
        if not product_id in new_df['product_id'].values:
             ...
             reviews = []
             new_df.loc[len(new_df)] = [product_id, product_title, product_category, product_parent, marketplace, reviews]
        ...
        new_df.loc[new_df['product_id'] == product_id, 'reviews'].apply(lambda x: x.append(review))
