...
import { Card, CardContent, CardMedia, CardActions, Button, Grid, TextareaAutosize } from '@material-ui/core';
...
const ProductReviewList = ({reviews}) => {
    ...
    return (
        <List component="nav" aria-label="main mailbox folders" style={{height:"100%", overflow:"auto"}}>
            {reviews.map((review) => (
...
const ReviewListItem = ({ review}) => {
    ...
    const {review_headline, review_body, star_rating, review_date, review_id} = review;
    ...
    // material-ui start 
    const stars = Array.from({length: star_rating}, () => <span><StarIcon style={{color: "#FFD700"}}/></span>);
    return (
        <ListItem button key={review_id} style={{height:"100%"}} variant="outlined">
